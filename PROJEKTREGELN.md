# PROJEKTREGELN.md — Gemeinsame Arbeitsanweisung
...
> Diese Datei liegt im Repo-Root und ist die gemeinsame Regelquelle für Codex und Claude Code. Beide lesen sie zu Beginn jeder Aufgabe vollständig; `AGENTS.md` und `CLAUDE.md` verweisen darauf. Wenn eine Anweisung der nutzenden Person dieser Datei widerspricht, weise freundlich darauf hin und frage nach.
>
...
- **Menü-Erklärung in der Hilfe** (der frühere Chip „Das Menü unten ist immer da.“ auf der Themenseite wurde am 19.09.2026 auf Wunsch entfernt: Das Menü ist dort ohnehin sichtbar)
...
**Eine dokumentierte Ausnahme (11.09.2026):** In den Handlungs-Ketten (`ketten-de.js`) steht der Handlungssatz `tun` **nur einmal** und ist auf allen drei Ebenen gleich. Grund: Er ist ein Merksatz, den die Person wiedererkennen soll — „Ich klicke nicht auf Links." heißt auf jeder Ebene dasselbe. Wer ihn je Ebene umformuliert, zerstört genau die Wiedererkennbarkeit, die ihn wirksam macht. Gestuft werden nur `warum` und `hilfe`, also die Begründung. Das gilt **nur** für dieses Feld; alles andere folgt der Regel unten.
...
| `topics.js` | 12 Themen, 132 Lektionen, Quiz, Merksätze. Basistext = **Leichte Sprache** (DIN-SPEC-33429-konform: keine Nebensätze, kein Konjunktiv, kein Passiv außer gewollten Betrugs-Zitaten). |
...
| `ketten-de.js` | `KETTEN` + `applyChains()` — **Handlungs-Ketten** (Task Analysis / Chaining, §3). Eine Handlung in 4–8 Einzelschritten, ein Schritt pro Bildschirm, **fehlerfrei** (es gibt keine falsche Antwort) mit **Prompt-Fading** über die Zahl der Durchgänge. Ergänzt die fünf Entscheidungs-Trainer um einen **Ablauf**-Trainer. Stand 11.09.2026: Pilot mit einem Thema (Datenschutz, „Dein Stopp-Plan"). Die Sätze sind wortgleich aus den `bullets` der Lektion „Was kann ich tun?" übernommen. Dazu `film` — vier Takte (Ruhe → Störung → Entscheidung → Auflösung) als Einstieg (seit 13.09.2026). |
| `app.js` | Renderer + Engine: Profile und Bild-Code (`PROFILE_BASE_KEYS`, `pKey`, `pickProfile`, `renderCodeAsk`, `CODE_LENGTH`), Rückkehr-Prüfung auf geteilten Geräten (`markAway`, `checkReturn`, `AWAY_LIMIT_MS`), Kopf-Zeichen (`updateHeaderSign`), Sprach-Ebenen, `resolveLessonContent`, `resolveSelfAssessment`, `pictoSrc`, Tab-Navigation (`navigateTab`, `setActiveTab`), Hash-Routing (`handleHash`, `rememberRoute`), Orientierung (`setOrientation`, `lastLessonContext`), Sitzungs-Fortschritt (`sessionDoneTopics`), Frage des Tages, `printSuccessBook`, `printQrCards`, Glossar (`GLOSSAR`), Vorlesen, Dark-Mode-Reaktion, `buildCompanionPanel`/`printCompanion`. |
...
7. **Veröffentlichen über GitHub Desktop** (Commit to main → Push origin) — erst nach **ausdrücklicher Freigabe**. Die nutzende Person ist nicht technisch; Schritte einfach erklären. Bei einer „lock file“-Meldung zuerst laufende Git-Vorgänge in GitHub Desktop, Claude Code und Codex prüfen. Eine Sperrdatei nur entfernen, wenn kein zugehöriger Git-Vorgang mehr läuft und sie nachweislich verwaist ist. In Worktrees den Pfad mit `git rev-parse --git-path index.lock` ermitteln; keinen festen `.git/index.lock`-Pfad voraussetzen.
8. Vorschau-/Hilfsdateien (`_vorschau-*.html`, `*.bak`) sind in `.gitignore` und gehören nicht ins Repo.

---

## 16a. Zusammenarbeit von Claude Code und Codex (seit 15.09.2026)

**Ein Projekt, getrennte Arbeitsordner:**

| Ordner | Rolle | Branch bei der Einrichtung |
|--------|-------|---------------------------|
| `lernplattform-live` | Geprüfte Hauptfassung, Zusammenführen und Veröffentlichung | `main` |
| `lernplattform-claude` | Arbeitskopie für Claude Code | `codex/claude-arbeit` |
| `lernplattform-codex` | Arbeitskopie für Codex / ChatGPT | `codex/codex-arbeit` |

Die Arbeitsordner sind Git-Worktrees neben dem bisherigen Projektordner. Jeder hat eigene Dateien und einen eigenen Branch. Die Git-Versionsgeschichte ist gemeinsam. Ein Worktree ist keine zusätzliche Datensicherung.

1. **Vor jeder Aufgabe** `pwd`, `git status --short --branch` und `git worktree list` prüfen. Im zugewiesenen Arbeitsordner arbeiten. Pro Arbeitsordner darf nur eine Sitzung schreiben. Läuft die Sitzung noch im Hauptordner, vor Quellcode-Änderungen in die passende Arbeitskopie wechseln. Die Einrichtung der Regeln und das ausdrücklich beauftragte Zusammenführen sind Ausnahmen.
2. **Klare Aufträge:** Jeder Agent bearbeitet einen abgegrenzten Auftrag. Änderungen an derselben Funktion möglichst nacheinander durchführen. Auch getrennte Dateien können fachlich zusammenhängen, insbesondere die drei Sprach-Ebenen (§2).
3. **Bestand schützen:** Fremde oder uncommittete Änderungen weder überschreiben noch zurücksetzen, automatisch stashen oder löschen. Keine Dateien zwischen Arbeitsordnern kopieren, um Unterschiede zu verdecken. Keine Branch-Wechsel, Commits oder Resets im Arbeitsordner des anderen Agenten ausführen.
4. **Aktuellen Stand übernehmen:** Vor einem neuen Auftrag prüfen, ob Änderungen aus `main` fehlen. Bereits integrierte Änderungen bei sauberem Arbeitsstand per Git übernehmen, bevor darauf aufgebaut wird. Bei offenen Änderungen zuerst den Auftrag abschließen oder eine ausdrückliche Übergabe machen. Nie unbemerkt auf einer veralteten Fassung weiterbauen.
5. **Gemeinsame Regeln:** Projektregeln ausschließlich in `PROJEKTREGELN.md` pflegen. `AGENTS.md` und `CLAUDE.md` bleiben kurze Verweise. Änderungen an den Regeln wie Code über Git zusammenführen und in die anderen Arbeitskopien übernehmen. Es gibt keine automatische Synchronisation zwischen Branches; laufende Sitzungen lesen übernommene Regeln neu ein.
6. **Übergabe:** Zum Abschluss Auftrag, Branch, geänderte Dateien, durchgeführte Prüfungen und offene Punkte knapp nennen. Dazu den Commit nennen, falls vorhanden; sonst ausdrücklich auf uncommittete Änderungen hinweisen. Gespräche und Erinnerungen werden zwischen den Programmen nicht automatisch übertragen.
7. **Zusammenführen nacheinander:** Eine beauftragte Sitzung führt fertige Änderungen in die Hauptfassung zusammen. Den kombinierten Stand nach §§16–17 prüfen; ein konfliktfreier Git-Merge ersetzt die Funktionsprüfung nicht. Bei App-Änderungen muss die endgültige `CACHE_VERSION` auch gegenüber der zuletzt veröffentlichten Fassung neu sein. Erst danach veröffentlichen, mit ausdrücklicher Freigabe über GitHub Desktop (§16.7).
8. **Lokale Vorschau:** Unterschiedliche Ports verwenden, zum Beispiel Hauptfassung `8000`, Claude `8001`, Codex `8002`. Den Server aus dem jeweiligen Arbeitsordner starten. So sind auch die Service-Worker-Caches der Vorschauen getrennt. Ignorierte lokale Vorschauen und Hilfsdateien werden nicht automatisch in neue Worktrees übernommen.
...
4. ~~**DigComp/ICF in der Begleit-Ebene:** explizite DigComp-Codes je Thema ergänzen?~~ **Erledigt (August 2026):** alle 12 Themen haben `kompetenzen` mit DigComp-2.2- und ICF-Codes (§7). **Entschieden (09.09.2026):** Die Codes stehen **nur in der Begleit-Ebene** der App (§7) — weder im Beobachtungsbogen (Abschnitt H entfernt) noch im Erfolgs-Heft (hatte nie Codes).
...
6. ~~**QR-Betrugs-Lektion auch im Kurz-Modus?**~~ **Erledigt:** „Vorsicht bei QR-Codes" liegt im Kurz-Modus. ~~**Neu offen dafür:** Der Kurz-Modus hält sein Versprechen nicht — er liefert 94 von 129 Lektionen (73 %).~~ **Ebenfalls erledigt (September 2026):** Der Kurz-Modus liefert heute **60 von 132 Lektionen (45 %)**, gleichmäßig **5 Schritte je Thema** — Start + 3 Kern-Lektionen + „Das merke ich mir". Die Themen-Seite beziffert die Menge sichtbar („Kurz — 5 Schritte", Prüfbericht B8), das Versprechen stimmt also wieder. Gesteuert wird das über **`topic.einfachLessons`** in `topics.js` (alle 12 Themen), nicht über `shortLessonIndexes`: Dieses Feld ist aus `topics.js` entfernt und in `getLessonsForMode()` (`app.js`) nur noch **Rückfallzweig** für ein künftiges Thema ohne eigene Kurzfassung. Der Zweig bleibt stehen; wer den Kurz-Modus ändert, ändert `einfachLessons`.
...
8. **Prüfgruppen-Katalog (offen):** Menü-Wörter („Mein Lernweg"), Lautsprecher-Symbol ohne Wort, Formel „eine Person, der du vertraust", Themen-Gruppen-Namen, Frage des Tages, KI-Einsamkeits-Satz, Frage-Muster-Sätze, QR-Karten, „Lernen starten"-Knopf, Leitsatz „So lernst du." mit den Stations-Wörtern „Merken" / „Prüfen" / „Handeln" (🧠 ✅ ➜, Station 0), „DEINE eine Sache für heute" (Station 5 — live heute als „Eine Sache für heute", app.js:4654/:4723), Stations-Etikett 🧠 Merken / ✅ Prüfen / ➜ Handeln im Themenfluss (Stationen 3–5), Liste „Prüfe dich selbst:" im Quiz-Ergebnis (Station 4), Knopf „Hilfe nochmal lesen" (Station 5, sensible Themen), Wegzeichen-Figuren v2 (winken/themen/lernweg/einstellungen/ruhig — transparent, statisch) — erkennt die Person die Seiten an den Figuren wieder? Passt die Rolle zur Figur? Sind die Haltungen eindeutig? Wo stören sie? (Konzept „Wegzeichen", 09.09.2026), First-Run-Etikette: „Mehr über dieses Angebot" (einklappte Angebots-Liste auf #start), „Fang hier an" (erste Kachel, erster Besuch), Screen-Titel „Wer lernt heute?" (Paket F, 09.09.2026), Bedien-Zeile Paket G: Pillen-Zustand „Liest vor" (statt „Stopp"), Chip-Kurzwort „Pause" (statt „Pause machen"), Sprach-Chip zeigt nur den Stufennamen; Block-Vorlese-Knöpfe leise und mit Maus erst bei Berührung des Blocks sichtbar (Beobachten: findet die Zielgruppe sie trotzdem?); Wegweiser ohne eigenen Hör-Knopf (die Pille liest den Ort-Satz zuerst); Abschluss-Aktionen als Chips und Text-Links (auch Einfach-Weg und Einfach-Quiz) (Paket G und H, 10.09.2026). **Geteiltes Gerät (11.09.2026):** Kopf-Zeichen mit „Das bist du" — erkennt die Person daran, dass sie gerade sie selbst ist? Auf dem Handy steht dort nur das Zeichen ohne Wort (ab 480 px kommt das Wort dazu) — reicht das? Wort „Bild-Code", Satz „Tippe deine 3 Bilder an. Immer in der gleichen Reihenfolge." — schafft die Zielgruppe eine Reihenfolge aus 3 Bildern, oder nur die Menge? Knopf „Ich weiß meinen Code nicht mehr" und „Code wegnehmen" — wird verstanden, dass dabei nichts verloren geht? Rückkehr-Prüfung nach 3 Minuten: Ist der Sprung zur Personen-Liste nachvollziehbar oder erschreckend? Ist die Schwelle richtig? Einstellungs-Schalter „Nur ich" / „Mehrere Personen". **Film vor der Kette (13.09.2026):** Erkennt die Person im Bild, was passiert — ohne den Satz zu lesen? Die beiden abstrakten Zeichen sind ausdrücklich zu prüfen: **pochender Punkt** als Druck und **Kreis-Welle** als Ruhe. Verstehen weniger als zwei Drittel sie (ISO-Schwelle 67 %), fliegen sie raus und werden durch konkretes Handeln ersetzt. Ebenso: Wird das **Gesicht** im zweiten Takt als besorgt gelesen — oder als böse? Im dritten Takt **wendet die Person das Gesicht vom Handy ab** — wird das als Zögern und Warten gelesen? (Eine Hand gab es im Entwurf, sie ist bewusst entfallen, siehe §18a.) Wörter: „Bild 1 von 4", „Weiter", „Und jetzt dein Plan", „Film überspringen". **Handlungs-Kette (11.09.2026):** Knopf „Gemacht" — versteht die Person, dass sie damit bestätigt, etwas getan zu haben, und nicht etwas beantwortet? Knopf „Plan üben" und später „Plan durchgehen". Der Satz „Du kannst nichts falsch machen." — beruhigt er oder verwirrt er? Zähler „Schritt 1 von 5". Knopf „Warum?" im zweiten Durchgang. Und der Kern der Methode: Merkt die Person beim zweiten Durchgang, dass weniger dasteht — und erlebt sie das als Fortschritt („ich kann das schon") oder als Verlust („die App hilft mir nicht mehr")? Der Satz „Beim nächsten Mal zeigt dir die App weniger." soll das abfedern; ob er trägt, muss die Gruppe sagen. **Rückmeldungen und Alltags-Übungen (15.09.2026):** Rückmeldungs-Wörter stehen zentral in `RUECKMELDUNG` (`app.js`) — „Diese Antwort passt" / „Schau dir die Erklärung an" statt „Genau richtig!" / „Fast!", „Thema geschafft" statt „Super gemacht!", „Eine Sache für heute" (ohne DEINE). Trägt die sachliche Rückmeldung — oder fehlt der Zielgruppe das Lob? Dazu die Übungen „Im Alltag üben" (`alltag-de.js`): Wörter „Zeig es mir", „Selbst ausprobieren", „Ich bin unsicher. Ich möchte Hilfe.", der Satz „Das ist eine Übung. Die Personen sind erfunden. Die App schickt nichts weg." — und: Wird der Unterschied zwischen Stumm schalten und Blockieren verstanden? **Frage-Muster A/B (18.09.2026):** Mit `?frage=neu` in der Adresse sehen alle Fragen mit richtiger Antwort gleich aus (Übung, Quiz, großes Quiz, Trainings-Postfach, Übungs-Handy, kurze Frage, Frage des Tages): Kopfzeile ✅ Prüfen + „Frage 1 von 11“ – Frage – leiser Knopf „Ich bin unsicher“ direkt darunter – Linie – „Tippe deine Antwort an.“ – Antworten. Ohne Zusatz bleibt die bisherige Fassung. Zu prüfen: Erkennt die Person schneller, wo die Frage steht und wo sie antworten muss? Wird der leisere Hilfe-Knopf noch gefunden (Prüfbefund V-5)? Hilft die sichtbare Aufforderung — oder ist sie überflüssig? Ergänzt (18.09.2026): Die Antworten stehen in einem getönten Antwort-Feld; auf dem Handy bleibt die Frage beim Scrollen oben stehen — hilft das oder irritiert die mitlaufende Zeile? Einschätzungen ohne Richtig und Falsch (Einstiegsfrage, „Wie ist es jetzt?“ am Ende) tragen im Entwurf das Etikett „💬 Deine Meinung“ statt ✅ Prüfen, keine Nummern, und VOR den Antworten den Satz „Hier gibt es kein Richtig und kein Falsch.“ mit der Aufforderung „Tippe an, was für dich stimmt.“ — Erkennt die Person den Unterschied zwischen Meinung und Prüfung? Nimmt der Satz die Angst?
...
## 18a. Bewegung und Animation (seit 13.09.2026)

Bewegung ist in dieser Plattform die **Ausnahme**, nicht das Gestaltungsmittel. Wer animiert, hält diese Regeln ein:

- **Zweckbindung.** Jede Animation gehört zu einer Lektion und einem Lernschritt. Ohne benennbaren didaktischen Zweck wird nicht animiert (§3 Kohärenz).
- **Nie der einzige Kanal.** Abstrakte Bildsprache hat für diese Zielgruppe geringe Ikonizität — die mittlere Verständlichkeit von Piktogrammen liegt empirisch bei 0,39, die ISO-Schwelle bei 0,67. Zu **jedem** Takt gehört ein Satz, und der steht in allen drei Ebenen (§2).
- **Kein Autoplay.** Jeder Takt wird angetippt. Damit greift WCAG 2.2.2 (Pause/Stopp bei Auto-Bewegung über 5 Sekunden) gar nicht erst — und die Forschung empfiehlt genau das: segmentierte Häppchen mit Weiter-Knopf statt einer durchlaufenden Animation, weil Bewegung flüchtige Information erzeugt.
- **Technik:** Inline-SVG mit CSS-Keyframes. Keine JS-Animation, keine externe Datei, kein Netzaufruf. Farben über CSS-Klassen und Token, **nie** als `fill="#…"`-Attribut — sonst bricht der Dunkelmodus (§10).
- **Im Bild verboten:** Text, Ziffern, Logos, App-Symbole, nachgebaute echte Bedienoberflächen, Kamerabewegung, harte Schnitte.
- **Tempo:** weiches Easing. `linear` ist verboten. Puls deutlich unter 3 Hz (WCAG 2.3.1).
- **`prefers-reduced-motion`:** Die Zustände bleiben gesetzt, nur die Übergänge fallen weg. Man blättert dann Standbilder durch — der Inhalt geht nicht verloren. Pflicht, nicht Kür.
- **Keine Hautflächen** (entschieden 13.09.2026). Die Plattform zeichnet Menschen seit jeher ohne Hautton: Kopf als Kreis in `--accent-soft` mit Kontur in `--accent`, siehe `pikto-person`. Es gibt bewusst **keine** Hautton-Variable.
- **Gesichter ja, aber nur im Film** (entschieden 13.09.2026). Zwei Augen, zwei Brauen, ein Mund-Strich, drei Zustände. Innere Zustände sind ohne Gesicht kaum ikonisch darstellbar. Die **Piktogramme bleiben gesichtslos** — ein Piktogramm ist ein stehendes Etikett, der Film zeigt einen Verlauf.
  **Achtung bei den Brauen:** nach innen-**unten** gezogene Brauen lesen sich als Wut. Bei Betrugs-Themen wäre das fatal, weil die Person es auf sich beziehen kann. Besorgt heißt: innere Enden nach **oben** (§3 Došen, §4).
- **Keine Hände zeichnen** (entschieden 13.09.2026, nach drei verworfenen Anläufen). Bei diesem Abstraktionsgrad wird jede gefüllte Hand-Silhouette zum Topfhandschuh, und ein einzelner ausgestreckter Finger zwischen angewinkelten las sich als **obszöne Geste** — auf einer Plattform für erwachsene Menschen mit Lern-Schwierigkeiten ein Desaster (§4). Innere Zustände wie Zögern zeigt stattdessen die **Person selbst**: abgewandtes Gesicht, veränderte Brauen, veränderter Mund. Das ist konkretes menschliches Verhalten statt abstrakter Metaphorik — und genau das, wofür Bewegung bei dieser Zielgruppe taugt.
- **Nebenwirkung beachtet:** Der Takt-Text hieß vorher „Deine Hand will schnell tippen." Das ist bildliche Sprache und in Leichter Sprache ausgeschlossen (§5). Jetzt: „Du willst schnell antworten. Du wartest."
