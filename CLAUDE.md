...
| `ketten-de.js` | `KETTEN` + `applyChains()` — **Handlungs-Ketten** (Task Analysis / Chaining, §3): eine Handlung in 4–8 Einzelschritten, ein Schritt pro Bildschirm, **fehlerfrei** (es gibt keine falsche Antwort), mit **Prompt-Fading** über die Zahl der Durchgänge. Dazu `film` — vier Takte (Ruhe → Störung → Entscheidung → Auflösung) als Einstieg. Die Sätze sind wortgleich aus den `bullets` der Lektion „Was kann ich tun?" übernommen. Stand 13.09.2026: Pilot mit einem Thema (Datenschutz, „Dein Stopp-Plan"). |
...
Die importierte Datei enthält die vollständigen, gemeinsamen Projektregeln.
Falls der Import nicht geladen wurde, lies `PROJEKTREGELN.md` vor der Aufgabe vollständig.
Claude Code arbeitet in `lernplattform-claude`; die Hauptfassung `lernplattform-live` dient dem beauftragten Zusammenführen und Veröffentlichen (§16a).
Projektregeln ausschließlich in `PROJEKTREGELN.md` pflegen.
