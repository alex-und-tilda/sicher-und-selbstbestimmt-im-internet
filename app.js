...
function getVisibleLearningTextForSpeech() {
  ...
  selectors.forEach(selector => {
    contentRoot.querySelectorAll(selector).forEach(element => {
      ...
      if (!text) return;
      if (/^(Zurück|Weiter|Startseite|Schließen|Anhören|Drucken)$/i.test(text)) return;
...
function safeReadVisibleContent() {
  if (!("speechSynthesis" in window)) {
    const live = document.querySelector("#liveRegion");
    if (live) live.textContent = "Vorlesen wird von diesem Browser nicht unterstützt.";
    return;
  }

  window.speechSynthesis.cancel();

  const text = getVisibleLearningTextForSpeech();
  if (!text) return;
  ...
  const live = document.querySelector("#liveRegion");
  if (live) live.textContent = "Der wichtige Inhalt wird vorgelesen.";
}

function ensureSafeListenButton() {
  if (document.querySelector(".safe-listen-button")) return;

  const button = document.createElement("button");
  button.type = "button";
  button.className = "safe-listen-button";
  button.textContent = "Anhören";
  button.setAttribute("aria-label", "Wichtigen Inhalt langsam anhören");
  button.addEventListener("click", safeReadVisibleContent);

  document.body.appendChild(button);
}

document.addEventListener("DOMContentLoaded", () => {
  ensureSafeListenButton();
});
