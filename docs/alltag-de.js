...
const ALLTAG_TEXT = {
  leicht: {
    ...
    next: 'Weiter', back: '← Zurück', choose: 'Andere Übung wählen',
...
/* Freigabe der Prüfgruppe (20.09.2026): Aus den Vergleichs-Entwürfen A und B
   wurde B der Standard – für alle Alltags-Übungen. Situation, Aufgabe und
   Rückmeldung kommen nacheinander auf derselben Seite, und JEDE Antwort
   bekommt eine Rückmeldung, auch die passende (§3: Rückmeldung auf jede
   Entscheidung). Der Schalter ?variante=a|b ist damit entfallen.
   Die Klassen heißen weiterhin alltag-v-… – das sind nur Namen. */
...
function renderAlltag(route) {
  ...
  if (!isCard) {
    content.innerHTML = buildAlltagPage({ id, scene, stepId, step, demo, t, offeredActions });
    focusContent();
    renderLegalFooter();
  ...
  /* Ab hier nur noch die Merk-Karte zum Ausdrucken – alle Übungsschritte
     laufen über buildAlltagPage (oben). */
...
function buildAlltagPage({ id, scene, stepId, step, demo, t, offeredActions }) {
  alltagCurrentRoute = alltagRoute(id, stepId, demo);
  const key = alltagCurrentRoute;
  const actions = step.actions || [];
  ...
  const focusMark = alltagPhase.moved ? ' id="alltag-phase-focus" tabindex="-1"' : '';
  ...
  const answerButtons = () => {
    if (isChoice) {
      const onclickFor = (a, i) => `alltagSetPhase('feedback', ${i})`;
  ...
  /* Nur im Datenschutz-Muster: die Hinweise von Alex und Tilda und die
     freiwillige Einschätzung am Ende. Sie steckten vorher nur im alten
     Seiten-Aufbau und würden sonst mit dem Wechsel verschwinden. */
  const privacyHinweis = id !== 'daten' || typeof privacyGuide !== 'function' ? ''
    : stepId === 'start' ? privacyGuide('alex')
    : (stepId === 'options' || stepId === 'transfer') ? privacyGuide('tilda') : '';
  const privacySchluss = id === 'daten' && step.end && typeof privacyClosing === 'function' ? privacyClosing() : '';
  ...
  if (step.end) {
    ...
    body = `<div${focusMark}>${label(t.situation, true)}</div>${heading}${goal}${text}${privacyHinweis}
      ${phone('')}
    ...
    const situationLabel = opensSituation ? label(t.situation) : '';
    body = `${cameFrom ? `<p class="alltag-v-chosen">${escapeHtml(t.yourAnswer)}: <strong>${escapeHtml(cameFrom[0])}</strong></p>` : ''}
      ${situationLabel}${heading}${goal}${text}${privacyHinweis}${guidance}
  ...
  return `${support}<div class="alltag-variant" data-readable="true">
    ${helpPanel}
